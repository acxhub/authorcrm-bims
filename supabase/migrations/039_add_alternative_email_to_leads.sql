-- Add a third email to leads (e.g. work, agent, family contact)
ALTER TABLE leads ADD COLUMN IF NOT EXISTS alternative_email varchar(255);

-- Include alternative_email in the lead-list global search RPC
  ...
  WITH filtered AS (
    ...
    WHERE
      ...
      AND (p_search IS NULL OR (
        ...
        OR l.alternative_email ILIKE '%' || p_search || '%'
        OR l.phone_number_1 ILIKE '%' || p_search || '%'
        OR l.alternative_phone_number ILIKE '%' || p_search || '%'
