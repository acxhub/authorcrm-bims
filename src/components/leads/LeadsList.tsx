...
import { Plus, Search, Filter, Mail, Phone, MoreHorizontal, Edit, Trash2, Eye, UserPlus, Calendar, X, RefreshCw, Pin, PinOff } from 'lucide-react';
  ...
  const handleRecycleLead = async (leadId: string) => {
    if (confirm('Are you sure you want to recycle this lead? It will be unassigned, tags will be cleared, and status will be reset.')) {
      try {
        ...
        toast({
          title: 'Lead recycled',
          description: 'Lead has been recycled successfully.',
        });
  ...
  const handleQuickAssign = async (leadId: string, assignedTo: string | null) => {
    ...
    try {
      ...
      toast({
        ...
        description: assignedTo ? `Assigned to ${assignedUser?.full_name}` : 'Lead is now unassigned',
      });
